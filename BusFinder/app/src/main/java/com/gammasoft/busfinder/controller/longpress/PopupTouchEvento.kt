...
class PopupTouchEvento internal constructor(private val mPressPopupInterface: PopupInterface,
    ...
    private val mLongPressRunnable = object: RunnableMotionEvent() {
        override fun run(){
            if(mCurrentPressStatus == STATUS_PRESSING && System.currentTimeMillis() - mStartPressTimestamp >= mLongPressDuration) startLongPress(startView, lastMotionEvent!!)
        }
    }
        ...
        when(motionEvent.action){
            ACTION_DOWN, ACTION_MOVE -> {
                when(mCurrentPressStatus){
                    ...
                    STATUS_PRESSING -> {
                        if(System.currentTimeMillis() - mStartPressTimestamp > mLongPressDuration) startLongPress(view, motionEvent)
                        else continuePress(motionEvent, getPressStatusPercentage())
                    }
